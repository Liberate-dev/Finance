...
export default function BillsPage() {
	const { bills, categories, addBill, updateBill, deleteBill, markBillPaid } =
		useFinanceStore();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const data = {
			...
			custom_days:
				formFrequency === "custom" ? parseInt(formCustomDays) : undefined,
		...
		try {
			...
			showToast(getErrorMessage(error, "Gagal menyimpan tagihan"), "error");
		}
	};
	...
	const handlePaid = async (id: string) => {
		try {
			...
			showToast(getErrorMessage(error, "Gagal menandai tagihan"), "error");
		}
	};
	...
	const handleDelete = async (id: string) => {
		try {
			...
			showToast(getErrorMessage(error, "Gagal menghapus tagihan"), "error");
		}
	};
	...
	return (
		<div className="page">
				...
				<div className="stat-card">
					...
					<div
						...
						style={{
							...
							color:
								sortedBills.filter((b) => getBillStatus(b) === "overdue")
									.length > 0
									? "var(--danger)"
									: "var(--success)",
						}}
			...
			<Modal
				...
				title={editId ? "Edit Tagihan" : "Tambah Tagihan"}
			>
				<form
					...
					<div className="form-row">
						<div className="input-group">
							...
							<select
								className="input"
								value={formFrequency}
								onChange={(e) => setFormFrequency(e.target.value as "weekly" | "monthly" | "yearly" | "custom")}
							>
