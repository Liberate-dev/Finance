...
export default function GoalsPage() {
	...
	const handleSave = async () => {
		...
		try {
			...
			showToast(getErrorMessage(error, "Gagal menyimpan target"), "error");
		}
	};
	...
	const handleDeposit = async () => {
		...
		try {
			...
			if (isComplete) {
				...
				showToast(
					`Berhasil menambah Rp ${Number(depositAmount).toLocaleString("id-ID")}`,
				);
			}
			...
			showToast(getErrorMessage(error, "Gagal menambah tabungan"), "error");
		}
	};
	...
	const handleDelete = async (id: string) => {
		try {
			...
			showToast(getErrorMessage(error, "Gagal menghapus target"), "error");
		}
	};
							...
							>
								...
								>
									...
									>
										...
										<div>
											<div
												style={{ fontWeight: 600, fontSize: "var(--font-md)" }}
											>
												{goal.name}
												{goal.is_completed && (
													<span
								...
								>
									...
									<span
										className={`badge ${goal.is_completed ? "badge-income" : "badge-default"}`}
									>
										{Math.round(progress)}%
									</span>
								</div>
